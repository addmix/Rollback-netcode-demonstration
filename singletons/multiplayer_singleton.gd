...
var network_update_frequency : int = 60
#how many seconds to interpolate
@export var network_interpolation_duration : float = 0.2
...
func _ready() -> void:
	...
	var timer := Timer.new()
	timer.wait_time = 1.0 / network_update_frequency
	timer.timeout.connect(on_network_update_timer_timeout)
	add_child(timer)
	timer.start()
...
func on_network_update_timer_timeout() -> void:
	network_update.emit()
...
func create_client(address: String, port: int, channel_count: int = 0, in_bandwidth: int = 0, out_bandwidth: int = 0, local_port: int = 0) -> void:
	...
	if error != OK:
		push_error("Error %s when creating client." % error)
	get_tree().get_multiplayer().multiplayer_peer = peer
